...
const blameMyNetwork = async (site, opts = {}) => {
	...
	try {
		...
		requestStack.internalConnectionTimings = internalNetworkResult ? internalNetworkResult.timings : -1;
	...
	if (requestStack.externalConnection) {
		if (requestStack.internalConnection) {
			...
			return _generateResponse(false, 'Blame your network. The external network can acess the site.');
		}
	} else {
		if (requestStack.internalConnection) {
